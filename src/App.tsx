...
import { allVideosData } from '../data'; // ★データをここで直接読み込む

// --- マイページ ---
// ★ propsから allVideosData, instructorsData を削除
export const MyPage = ({ userInfo, setUserInfo, likedVideos, toggleLike, setView, setPlayingVideo }: any) => {
  ...
  return (
    <div className="max-w-2xl mx-auto p-8 bg-white my-12 rounded-3xl shadow-lg border border-slate-100 font-sans animate-in slide-in-from-bottom-4">
      ...
      <div className="space-y-6">
        ...
        <button className="w-full bg-slate-300 text-slate-500 font-bold text-lg py-4 rounded-xl cursor-pointer hover:bg-slate-400 transition transform" onClick={handleInteraction}>応募する（無料）</button>
      </div>
    </div>
  );
};
...
export const RegisterPage = ({ setView, onRequireAuth }: any) => (
  <div className="max-w-md mx-auto p-10 bg-white my-16 rounded-3xl shadow-2xl border border-slate-100 text-center font-sans animate-in zoom-in-95">
    ...
    <div className="space-y-4">
      <button onClick={onRequireAuth} className="w-full bg-[#1877f2] text-white font-bold py-3.5 rounded-xl flex items-center justify-center gap-3 hover:bg-[#0d65d9] transition shadow-sm"><svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg> Facebookで登録</button>
      <button onClick={onRequireAuth} className="w-full bg-slate-800 text-white font-bold py-3.5 rounded-xl flex items-center justify-center gap-3 hover:bg-slate-900 transition shadow-sm"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg> メールアドレスで登録</button>
    </div>
