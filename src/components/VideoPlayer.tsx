...
export const VideoPlayer = ({ video, onClose, likedVideos, toggleLike, setView }: any) => {
  if (!video) return null;

  const instructor = instructorsData[video.instructor];

  return (
    <div className="bg-slate-900 text-white animate-in fade-in slide-in-from-bottom-4 duration-500 min-h-[calc(100vh-64px)] pb-12">
      <div className="max-w-6xl mx-auto">
        <div className="p-4 flex justify-end">
          <button onClick={onClose} className="flex items-center gap-1 text-slate-400 hover:text-white transition">
            <X size={24} /> 閉じる
          </button>
        </div>

        <div className="aspect-video w-full bg-black relative shadow-2xl overflow-hidden">
          ...
          {video.isPremium ? (
            <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-6 bg-black/60 backdrop-blur-sm">
              <Lock className="w-16 h-16 text-yellow-400 mb-6 drop-shadow-lg" />
              <h3 className="text-3xl font-bold mb-4">この動画は会員限定です</h3>
              <p className="text-slate-300 mb-8 text-lg max-w-lg">
                月額会員になると、{video.instructor}先生のすべての動画が見放題になります。<br/>
                地域の知恵と技を、もっと深く学びませんか？
              </p>
              <button className="bg-gradient-to-r from-yellow-500 to-orange-600 text-white text-xl font-bold px-10 py-4 rounded-full shadow-lg hover:scale-105 transition transform">
                今すぐ無料体験を始める
              </button>
            </div>
          ) : (
            <div className="absolute inset-0 flex items-center justify-center group cursor-pointer">
              <div className="w-24 h-24 bg-white/20 rounded-full flex items-center justify-center backdrop-blur-md group-hover:scale-110 transition duration-300 border-2 border-white/50">
                <Play className="fill-white text-white translate-x-1" size={48} />
              </div>
              <div className="absolute bottom-4 left-4 bg-black/70 px-3 py-1 rounded text-sm font-mono">
        ...
        <div className="p-6 md:p-10 grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="md:col-span-2 space-y-6">
            <div>
              ...
              <h1 className="text-3xl md:text-4xl font-bold mb-4 leading-tight">{video.title}</h1>
            ...
            <div className="flex gap-4">
              ...
              <button className="flex-1 py-3 rounded-xl font-bold flex items-center justify-center gap-2 bg-slate-800 text-slate-300 hover:bg-slate-700 transition">
          ...
          <div className="space-y-6">
            <div className="bg-white/5 p-6 rounded-2xl border border-white/10">
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  ...
                  <h3 className="font-bold text-lg">{video.instructor}</h3>
                  <p className="text-xs text-slate-400">{instructor?.title}</p>
                </div>
              </div>
              <button onClick={() => { onClose(); setView('instructor'); }} className="w-full bg-white/10 hover:bg-white/20 text-white py-2 rounded-lg text-sm font-bold transition">
                プロフィールを見る
              </button>
            </div>
            ...
            <div className="bg-gradient-to-br from-teal-900 to-emerald-900 p-6 rounded-2xl border border-teal-700/50 shadow-lg">
              ...
              <button className="w-full bg-white text-teal-900 font-bold py-3 rounded-xl hover:bg-teal-50 transition flex items-center justify-center gap-2">
